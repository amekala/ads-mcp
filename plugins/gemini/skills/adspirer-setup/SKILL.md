---
name: adspirer-setup
description: Set up your brand workspace — connect to Adspirer, scan brand docs, pull campaign data, and create GEMINI.md. Use on first time in a new brand folder, or when the user wants to refresh brand context.
---
...
- No `GEMINI.md` in the project root: create it (the user explicitly asked for setup, which covers this).
- `GEMINI.md` exists and is already an Adspirer brand workspace: update it in place, preserving any edits the user made.
- `GEMINI.md` exists and is anything else (e.g. a software project's instructions): NEVER overwrite, restructure, or delete it. Ask the user whether to append a clearly-marked `## Adspirer Brand Context` section at the end, or to skip the file and keep brand context in the conversation only. Do not touch the file until they answer.
...
Create `STRATEGY.md` at the project root alongside GEMINI.md:
...
End with:
"Your brand workspace is set up! I've saved everything to GEMINI.md.
