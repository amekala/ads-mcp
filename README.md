  ...
  Remote Model Context Protocol (MCP) server for campaign planning, research, and cross-platform ad
  creation. Supports Google Ads Search & Performance Max and TikTok at launch, with additional
  networks planned.
  ...
  - Create Google Ads Search and Performance Max campaigns, and TikTok campaigns, end-to-end from
  within MCP-capable clients
  - Real-time progress updates for long-running operations (5-30 seconds)

  ## Tools

  ### Asset Management
  - `help_user_upload` — Returns clear instructions for providing direct media links suitable for
  ingestion
  - `validate_and_prepare_assets` — Downloads and validates media from provided URLs; returns an
  `asset_bundle_id`. **Streams progress** when supported by the client (typically 5-15 seconds for
  5-10 images)

  ### Campaign Creation
  - `create_pmax_campaign` — Atomic PMax creation (uses validate-then-commit pattern). **Streams 
  progress** when supported (typically 15-30 seconds)
